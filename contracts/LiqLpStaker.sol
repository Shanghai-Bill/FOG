...
abstract contract RewardContract {
    function getPLASMAbalance() external virtual view returns (uint256);

    function givePLASMAbalance(address recipient, uint256 amount)
        external
        virtual
        returns (bool);
}

contract FluxCampacitor is Ownable {
    using SafeMath for uint256;

    LiqContract private _LiqContract; // Liq token contract
    ...
    constructor(LiqContract LiqContract, address devWallet) public {
        _LiqContract = LiqContract;
        _devWallet = devWallet;
    }

    function stake(uint256 amount) public {
        require(
            _LiqContract.transferFrom(_msgSender(), address(this), amount),
            "FluxCampacitor: stake failed."
        );

        uint256 StakedAmount = amount;
        uint256 currentTimestamp = uint256(now);

        if (_stakers.length == 0) _devLastClaimTimestamp = currentTimestamp;

        if (_stakeMap[_msgSender()].stakedAmount == 0) {
            _stakers.push(_msgSender());
            _stakeMap[_msgSender()].lastClaimTimestamp = currentTimestamp;
        } else {
            _stakeMap[_msgSender()].rewardAmount = _stakeMap[_msgSender()]
                .rewardAmount
                .add(calcReward(_msgSender(), currentTimestamp));
            _stakeMap[_msgSender()].lastClaimTimestamp = currentTimestamp;
        }

        _stakeMap[_msgSender()].stakedAmount = _stakeMap[_msgSender()]
            .stakedAmount
            .add(StakedAmount);
    ...
    function unstake(uint256 amount) public {
        require(
            ...
            "FluxCampacitor: unstake amount exceededs the staked amount."
        );
        ...
        require(
            _LiqContract.transfer(_msgSender(), amount),
            "FluxCampacitor: unstake failed."
        );
    ...
    function claim() public {
        uint256 currentTimestamp = uint256(now);
        uint256 rewardAmount = _stakeMap[_msgSender()].rewardAmount.add(
            calcReward(_msgSender(), currentTimestamp)
        );
        _stakeMap[_msgSender()].lastClaimTimestamp = currentTimestamp;

        require(
            _rewardContract.givePLASMAbalance(_msgSender(), rewardAmount),
            "FluxCampacitor: claim failed."
        );
        ...
        if (currentTimestamp.sub(_devLastClaimTimestamp) >= 86400) {
            ...
            require(
                _rewardContract.givePLASMAbalance(_devWallet, rewardAmount),
                "FluxCampacitor: dev reward claim failed."
            );
    ...
    function calcReward(address staker, uint256 currentTimestamp)
        private
        view
        returns (uint256)
    {
        if (_totalStakedAmount == 0) return 0;
        uint256 rewardPoolBalance = _rewardContract.getPLASMAbalance();
        uint256 passTime = currentTimestamp.sub(
            _stakeMap[staker].lastClaimTimestamp
        );
        uint256 rewardAmountForStakers = rewardPoolBalance
            .mul(_rewardPortion)
            .div(100)
            .mul(_rewardFee)
            .div(100);
        uint256 rewardAmount = rewardAmountForStakers
            .mul(passTime)
            .div(86400)
            .mul(_stakeMap[staker].stakedAmount)
            .div(_totalStakedAmount);
        return rewardAmount;
    }

    function calcDevReward(uint256 currentTimestamp)
        private
        view
        returns (uint256)
    {
        ...
        uint256 rewardAmount = rewardPoolBalance
            .mul(_rewardPortion)
            .div(100)
            .mul(uint256(100).sub(_rewardFee))
            .div(100)
            .mul(passTime)
            .div(86400);
        return rewardAmount;
    ...
    function getRewardPoolBalance() external view returns (uint256) {
        return _rewardContract.getPLASMAbalance();
    }

    /**
     * Get last claim timestamp
     */
    function getLastClaimTimestamp(address staker)
        external
        view
        returns (uint256)
    {
    ...
    function getStakedRank(address staker) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < _stakers.length; i++) {
            if (
                _stakers[i] != staker &&
                senderStakedAmount < _stakeMap[_stakers[i]].stakedAmount
            ) rank = rank.add(1);
        }
        return rank;
    }
     ...
     */
    function setRewardContract(RewardContract rewardContract)
        external
        onlyOwner
        returns (bool)
    {
        require(
            ...
            "FluxCampacitor: reward contract address should not be zero address."
        );
